...
# This script will run N copies of PassiveWebMoney using N proxies provided as list in the file passed as argument --proxies-file <filename> , by default it will use proxies.txt
# It will create and subflder named "pwm_proxy_instances" and a subfolder with same name of the original COMPOSE_PROJECT_NAME<unique_suffix> the it will copy all the files from the root folder to the subfolder created for each instance and named like "pwm-<COMPOSE_PROJECT_NAME><unique_suffix>/<DEVICE_NAME><unique_suffix>" deriving this from the original instance running with or without proxy in the root folder that uses the original .env and docker-compose.yaml files
...
$instancesDir = Join-Path -Path $rootDir -ChildPath "pwm_proxy_instances"
...
if (Test-Path -Path $instancesDir) {
    ...
    foreach ($proxy in $proxies) {
        ...
        $instanceName = "pwm_${composeProjectName}-${deviceName}-${uniqueSuffix}"
        ...
        # Update the ports present in the .env file like MYSTNODE_DASHBOARD_PORT PWM_DASHBOARD_PORT and so on increasing their value by $num_instances_created+1
